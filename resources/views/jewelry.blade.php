    ...
    <div class="py-12">
        <div class="flex flex-col gap-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div
                ...
                <form action="{{route("jewelry.store", ["category"=>$category])}}" method="post"
                      enctype="multipart/form-data" class="flex flex-col p-6 text-lg gap-4">
                    @csrf
                    <div class="flex gap-4">
                        <div class="w-1/4 flex flex-col gap-2">
                            <label class="text-white" for="img">Основное изображение</label>
                        ...
                        <div class="w-1/4 flex flex-col gap-2">
                            <label class="text-white" for="alternateImg">Альтернативное изображение</label>
                        ...
                        <div class="w-1/4 flex flex-col gap-2">
                            ...
                            <select name="type" class="rounded-lg h-full border-2 border-transparent" id="type" required>
                                ...
                                <option value="necklace">Колье / Ожерелье</option>
                                <option value="hairstyle">Прическа</option>
                            </select>
                            @error("type")
                            <p class="text-red-500">{{$message}}</p>
                            @enderror
                    ...
                    <button type="submit" class="bg-fuchsia-600 text-white p-4 rounded-lg">Добавить украшение / причёску</button>
                </form>
            </div>
            <div
                class="grid @if(count($jewelries) > 0) grid-cols-4 @endif text-lg gap-4 p-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">

                @if(count($jewelries) > 0)
                    @foreach($jewelries as $item)
                        <div
                            ...
                            <div
                                ...
                                <a href="{{route("jewelry.edit", ["jewelry" => $item, "category" => $category])}}"
                                   class="rounded">
                                ...
                                <form action="{{route("jewelry.destroy", ["jewelry" => $item])}}" method="post"
                                      class="h-8 rounded">
                                    @csrf
                                    @method("delete")
                                    <button>
                            ...
                            <img src="{{asset($item->img)}}" class="h-64 w-full p-6" alt="Картинка украшения или прически">
                        </div>
                    @endforeach
                @else
                    <p class="text-white">Украшения и прически в категории не добавлены</p>
                @endif
